...
export class LawyerStateService extends SimpleStore<LawyerState> {
    ...
    private continueLoadingLawyers() {
        ...
        combineLatest([this.select('search'), this.select('pageNumber'), this.select('pageSize'), this.select('orderBy')])
            .pipe(
                debounceTime(200),
                ...
                switchMap(([search, pageNumber, pageSize, orderBy]) => {
                    ...
                    return this.lawyerApiService.getAllLawyers({ search, page, size, orderBy })
                })
            ).subscribe({
                next: (res) => {
                    this.setState({
