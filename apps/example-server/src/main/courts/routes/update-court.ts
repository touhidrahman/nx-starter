...
export const updateCourtHandler: AppRouteHandler<typeof updateCourtRoute> = async (c) => {
    ...
    try {
        ...
        if (!existingCourt) {
            return c.json(jsonResponse({}, 'Court not found', NOT_FOUND), NOT_FOUND)
        }

        const updatedCourt = await courtsService.updateCourt(courtId, body)
        return c.json(jsonResponse(updatedCourt, 'Court updated successfully', OK), OK)
    } catch (error) {
        if (error instanceof z.ZodError) {
            return c.json(jsonResponse({}, 'Invalid court data', BAD_REQUEST), BAD_REQUEST)
        }
        console.error('Error updating court:', error instanceof Error ? error.message : 'Unknown error')
