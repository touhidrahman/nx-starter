...
export class CourtsService {
    // Find all courts
    async findAllCourts() {
        const courts = await db.select().from(courtsTable).limit(100)
    ...
    async updateCourt(id: string, court: Partial<InsertCourt>) {
        return db
            .update(courtsTable)
            .set(court)
            .where(eq(courtsTable.id, id))
    ...
    async deleteCourt(id: string) {
        return db.delete(courtsTable).where(eq(courtsTable.id, id)).returning()
    }
