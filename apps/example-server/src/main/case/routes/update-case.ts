...
export const updateCaseHandler: AppRouteHandler<typeof updateCaseRoute> = async (c) => {
    ...
    try {
        ...
        return c.json(jsonResponse(updatedCase, 'Case updated successfully', OK), OK)
    } catch (error) {
        if (error instanceof z.ZodError) {
            return c.json(jsonResponse({}, 'Invalid case data', BAD_REQUEST), BAD_REQUEST)
        }
        console.error('Error updating case:', error instanceof Error ? error.message : 'Unknown error')
