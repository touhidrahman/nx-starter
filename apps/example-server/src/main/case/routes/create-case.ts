...
export const createCaseHandler: AppRouteHandler<typeof createCaseRoute> = async (c) => {
    const body = c.req.valid('json')

    try {
        const newCase = await caseService.createCase(body)
        return c.json(jsonResponse(newCase, 'Case created successfully', CREATED), CREATED)
    } catch (error) {
        if (error instanceof z.ZodError) {
            return c.json(jsonResponse({}, 'Invalid case data', BAD_REQUEST), BAD_REQUEST)
        }
        console.error('Error creating case:', error instanceof Error ? error.message : 'Unknown error')
