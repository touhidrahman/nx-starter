...
export const updateAdminUserHandler: AppRouteHandler<typeof updateAdminUserRoute> = async (c) => {
    ...
    try {
        ...
        console.error('Error promoting user to admin:', error instanceof Error ? error.message : 'Unknown error')
        if (error instanceof Error) console.error(error.stack)
        return c.json(
            jsonResponse({}, 'Failed to promote user to admin', INTERNAL_SERVER_ERROR),
