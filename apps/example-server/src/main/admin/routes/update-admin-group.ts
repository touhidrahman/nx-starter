...
export const updateAdminGroupHandler: AppRouteHandler<typeof updateAdminGroupRoute> = async (c) => {
    ...
    try {
        ...
        if (!existingGroup) {
            return c.json(jsonResponse({}, 'Admin group not found', NOT_FOUND), NOT_FOUND)
        }

        const updatedGroup = await adminGroupsService.updateGroup(groupId, body)
        return c.json(jsonResponse(updatedGroup[0], 'Admin group updated successfully', OK), OK)
    } catch (error) {
        if (error instanceof z.ZodError) {
            return c.json(jsonResponse({}, 'Invalid admin group data', BAD_REQUEST), BAD_REQUEST)
        }
        console.error('Error updating admin group:', error instanceof Error ? error.message : 'Unknown error')
        if (error instanceof Error) console.error(error.stack)
        return c.json(
            jsonResponse({}, 'Failed to update admin group', INTERNAL_SERVER_ERROR),
