...
export const createAppointmentRoute = createRoute({
    ...
    responses: {
        [CREATED]: ApiResponse(zSelectAppointment, 'Appointment created successfully'),
...
> = async (c) => {
    ...
    try {
        ...
        return c.json(jsonResponse(newAppointment, 'Appointment created successfully', CREATED), CREATED)
    } catch (error) {
        ...
        return c.json(
            jsonResponse({}, 'Failed to create appointment', INTERNAL_SERVER_ERROR),
