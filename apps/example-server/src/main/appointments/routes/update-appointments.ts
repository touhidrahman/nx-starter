...
import { OK, BAD_REQUEST, INTERNAL_SERVER_ERROR, NOT_FOUND } from 'stoker/http-status-codes'
...
export const updateAppointmentRoute = createRoute({
    ...
    responses: {
        [OK]: ApiResponse(zSelectAppointment, 'Appointment updated successfully'),
...
export const updateAppointmentHandler: AppRouteHandler<typeof updateAppointmentRoute> = async (c) => {
    ...
    try {
        ...
        if (!existingAppointment) {
            return c.json(jsonResponse({}, 'Appointment not found', NOT_FOUND), NOT_FOUND)
        }

        const updatedAppointment = await appointmentService.updateAppointment(appointmentId, body)
        return c.json(jsonResponse(updatedAppointment, 'Appointment updated successfully', OK), OK)
    } catch (error) {
        if (error instanceof z.ZodError) {
            return c.json(jsonResponse({}, 'Invalid appointment data', BAD_REQUEST), BAD_REQUEST)
        }
        console.error('Error updating appointment:', error instanceof Error ? error.message : 'Unknown error')
        if (error instanceof Error) console.error(error.stack)
        return c.json(
            jsonResponse({}, 'Failed to update appointment', INTERNAL_SERVER_ERROR),
